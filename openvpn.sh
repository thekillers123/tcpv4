...
# Sanity check
if [[ $(id -g) != "0" ]] ; then
    die "â¯â¯â¯ Script must be run as root."
fi

if [[  ! -e /dev/net/tun ]] ; then
    die "â¯â¯â¯ TUN/TAP device is not available."
fi

dpkg -l openvpn > /dev/null 2>&1
if [[ $? -eq 0 ]]; then
    die "â¯â¯â¯ OpenVPN is already installed."
fi

# Install openvpn
ok "â¯â¯â¯ apt-get update"
apt-get update -q > /dev/null 2>&1
ok "â¯â¯â¯ apt-get install openvpn curl openssl"
...
ok "â¯â¯â¯ Generating Server Config"
...
ok "â¯â¯â¯ Generating Client Config"
...
ok "â¯â¯â¯ Your client config is available at /etc/openvpn/client.ovpn"
ok "â¯â¯â¯ All done!"
